...
server <- function(input, output, session) {
  ...
  # Initialize dm_data from a server-side file
  tryCatch({
    dm_data(haven::read_xpt("./data/dm.xpt"))
  }, error = function(e) {
    # Handle the error
    print(paste("Failed to read dm.xpt: ", e))
  })
  ...
  # Reactive value for labs data
  lb_data <- reactiveVal()
  tryCatch({
    lb_data(haven::read_xpt("./data/lb.xpt"))
  }, error = function(e) {
    # Handle the error
    print(paste("Failed to read lb.xpt: ", e))
  })
  observe({
    req(lb_data())
      # Update selectInput
