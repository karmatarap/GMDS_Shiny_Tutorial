...
server <- function(input, output, session) {
  ...
  output$dm_plot <- renderPlot({
    ...
    # Fill in the code below to create a ggplot2 histogram based on the selected variable.
    # Use aes_string() to map the variable to the x-axis.
    # Customize the plot with appropriate binwidth, fill color, and title.
    # Hint: Use geom_histogram() to create the histogram.
    
    # ggplot(data, aes_string(x = _________________)) +
    #   geom_histogram(binwidth = __________, fill = '__________', alpha = 0.7) +
    #   labs(title = paste("Histogram of", _______________))
    } else {
    # Same as previous step, but now we want to use a bar plot for categories
    # Hint: user geom_bar() to create bar plots
