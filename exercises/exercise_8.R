...
ui <- fluidPage(
  ...
  navbarPage(title = "Welcome to the GMDS RShiny Workshop by BMS!",
             ...
             # TODO:
             # Add a tabpanel for labs
             # add File input for labs
             # add a selectInput for the labs test
             # add an output for the plotly plot
  )
)
...
server <- function(input, output, session) {
  ...
  # TODO: Initialize an empty reactive for lb_data
  ...
  observeEvent(input$lbFile, {
    ...
    if (tools::file_path_sans_ext(basename(input$lbFile$name)) != "lb") {
      ...
      # TODO: Update test choices based on the uploaded dataset
  ...
  output$lb_plot <- renderPlotly({
    ...
    p <- ggplot(summary_data, aes(x = VISIT, y = Mean)) +
      ...
      # TODO: play around with the colors, how whacky can you make them??
